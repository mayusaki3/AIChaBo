# -*- coding: utf-8 -*-
"""
COMMON-CHAT:T06-02 unittest suite
...
class TextSplitEdgesTest(unittest.TestCase):
    ...
    def test_06_non_int_raises(self):
        # [COMMON-CHAT:T06-02-06] max_chars が int 以外 → TypeError
        with self.assertRaises(TypeError):
            self.split("abc", max_chars="10")  # type: ignore

    def test_07_super_long_single_token(self):
        # [COMMON-CHAT:T06-02-07] 超長単語: 120 文字を 50 ずつハード分割
        text = "X" * 120
...
if __name__ == "__main__":
    mapping = {
        "test_01_min_unit": ("COMMON-CHAT:T06-02-01", "max_chars=1 の極小分割"),
        "test_02_period_space_sentence": ("COMMON-CHAT:T06-02-02", '英文 ". " 文末分割'),
        "test_03_crlf_newlines": ("COMMON-CHAT:T06-02-03", "CRLF 混在の改行処理"),
        "test_04_exact_boundary": ("COMMON-CHAT:T06-02-04", "max_chars ジャスト境界"),
        "test_05_negative_raises": ("COMMON-CHAT:T06-02-05", "負の値/0 → ValueError"),
        "test_06_non_int_raises": ("COMMON-CHAT:T06-02-06", "非 int → TypeError"),
        "test_07_super_long_single_token": ("COMMON-CHAT:T06-02-07", "超長トークンのハード分割"),
    }
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(TextSplitEdgesTest)
    run_unittest_suite("COMMON-CHAT:T06-02 common/chat/textsplit", suite, mapping)
