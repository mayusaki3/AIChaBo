# -*- coding: utf-8 -*-
"""
COMMON-CHAT:T07-02 unittest suite
...
class ContinuationEdgesTest(unittest.TestCase):
    ...
    def test_01_max_steps_zero(self, mock_chat_loop):
        # [COMMON-CHAT:T07-02-01] max_steps=0 → chat_loop 呼ばれない
    ...
    def test_02_empty_chunks(self, mock_chat_loop):
        # [COMMON-CHAT:T07-02-02] 空文字のみ → 呼ばれない
        mock_chat_loop.chat.return_value = "ok"
    ...
    def test_03_textsplit_raises(self, mock_chat_loop, mock_textsplit):
        # [COMMON-CHAT:T07-02-03] textsplit 例外 → 呼ばれない
        mock_chat_loop.chat.return_value = "ok"
    ...
    def test_04_none_policy(self, mock_chat_loop):
        # [COMMON-CHAT:T07-02-04] policy=None でも落ちない
    ...
    def test_05_runs_once(self, mock_chat_loop):
        # [COMMON-CHAT:T07-02-05] 単一チャンク → 1 回だけ呼ばれる
        mock_chat_loop.chat.return_value = "ok"
...
if __name__ == "__main__":
    mapping = {
        "test_01_max_steps_zero": ("COMMON-CHAT:T07-02-01", "max_steps=0 → 呼ばれない"),
        "test_02_empty_chunks": ("COMMON-CHAT:T07-02-02", "空チャンク → 呼ばれない"),
        "test_03_textsplit_raises": ("COMMON-CHAT:T07-02-03", "textsplit 例外 → 呼ばれない"),
        "test_04_none_policy": ("COMMON-CHAT:T07-02-04", "policy=None でも落ちない"),
        "test_05_runs_once": ("COMMON-CHAT:T07-02-05", "単一チャンクは1回呼び出し"),
    }
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(ContinuationEdgesTest)
    run_unittest_suite("COMMON-CHAT:T07-02 common\chat\continuation", suite, mapping)
