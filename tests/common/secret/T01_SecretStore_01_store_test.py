...
出力:
  ✅/❌ と [M01:T01-01-xx] を先頭に持つ行 + SUMMARY（共通レポータ）
注意:
  - 実ファイルへの影響を避けるため、テスト側で SecretStore の保存先を
    テンポラリに差し替えるスイート（M01:T01-02, M01:T01-03, M01:T01-04）も別途用意している。
...
if __name__ == "__main__":
    ...
    print(f"=== {SUITE_TITLE} ===")
    rep = Reporter(SUITE_TITLE)

    # [M01:T01-01-01] user鍵: put-get roundtrip（基本往復）
    ...
    # [M01:T01-01-02] server鍵: put-get / has_server_any_key / delete（基本往復＋存在確認＋削除）
    ...
    # [M01:T01-01-03] user鍵: 2並列 put → LWW（最後の書き込みが勝つ）＆部分破損なし
    ...
    # [M01:T01-01-04] server鍵: 3並列 put → LWW & 破損なし
